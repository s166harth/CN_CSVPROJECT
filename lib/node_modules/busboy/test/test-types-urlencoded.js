...
const tests = [{
  ...
  source: [Array.from(transcode(Buffer.from('foo'), 'utf8', 'utf16le')).map(n => `%${n.toString(16).padStart(2, '0')}`).join(''), '=', Array.from(transcode(Buffer.from('😀!'), 'utf8', 'utf16le')).map(n => `%${n.toString(16).padStart(2, '0')}`).join('')],
  expected: [['foo', '😀!', {
    nameTruncated: false,
    valueTruncated: false,
    encoding: 'UTF-16LE',
  ...
  source: ['foo=<', Array.from(transcode(Buffer.from('©:^þ'), 'utf8', 'latin1')).map(n => `%${n.toString(16).padStart(2, '0')}`).join('')],
  expected: [['foo', '<©:^þ', {
    nameTruncated: false,
    valueTruncated: false,
    encoding: 'ISO-8859-1',
